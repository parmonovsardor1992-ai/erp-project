        ...
        <section className="overflow-auto rounded border border-line bg-white">
          <div className="flex flex-wrap gap-2 border-b border-line bg-panel p-2">
            <SelectField value={filters.customerId} onChange={(event) => setFilters((current) => ({ ...current, customerId: event.target.value }))}>
              ...
              {(dictionaries.data?.customers ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </SelectField>
          ...
          <table className="erp-table">
            ...
            <tbody>
              {isLoading && <tr><td colSpan={13}>{ru.common.loading}</td></tr>}
              {(data ?? []).map((order) => (
                <tr key={order.id}>
                  <td>{order.number}</td>
                  ...
                  <td className="text-right font-medium">{money(Number(order.orderDebt ?? 0), order.currencyCode)}</td>
                  <td>
                    <div className="flex gap-2">
                      <button className="text-[var(--accent-text)]" type="button" onClick={() => edit(order)}>Редактировать</button>
                      <button className="text-danger" type="button" onClick={() => window.confirm('Удалить заказ?') && remove.mutate(order.id)}>Удалить</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <form onSubmit={submit} className="rounded border border-line bg-white">
          ...
          <div className="grid gap-3 p-3">
            ...
            <SelectField value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))}>
              {Object.entries(orderStatusRu).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
            </SelectField>
            <Button disabled={create.isPending || update.isPending || !form.counterpartyId || !form.number} type="submit">{editingId ? 'Сохранить' : 'Добавить'}</Button>
